...
func (s *Server) handleHTTL(req *memapv1.Request) *memapv1.Response {
	ttl, err := s.manager.HTTL(req.GetNamespaceName(), req.GetKey())
	if err != nil {
		return errResponse(err)
	}
...
func (s *Server) handleHEXIST(req *memapv1.Request) *memapv1.Response {
	exists, err := s.manager.HExists(req.GetNamespaceName(), req.GetKey())
	if err != nil {
		return errResponse(err)
	}
	if exists {
...
func (s *Server) handleHLEN(req *memapv1.Request) *memapv1.Response {
	len, err := s.manager.HLen(req.GetNamespaceName(), req.GetKey())
	if err != nil {
		return errResponse(err)
	}
...
func (s *Server) handleHKEYS(req *memapv1.Request) *memapv1.Response {
	keys, err := s.manager.HKeys(req.GetNamespaceName(), req.GetKey())
	if err != nil {
		return errResponse(err)
	}
...
func (s *Server) handleHVALS(req *memapv1.Request) *memapv1.Response {
	values, err := s.manager.HValues(req.GetNamespaceName(), req.GetKey())
	if err != nil {
		return errResponse(err)
	}
